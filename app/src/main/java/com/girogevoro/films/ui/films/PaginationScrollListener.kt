...
abstract class PaginationScrollListener (private var layoutManager: LinearLayoutManager) : RecyclerView.OnScrollListener() {
    ...
    override fun onScrolled(recyclerView: RecyclerView, dx: Int, dy: Int) {
        ...
        if (!isLoading() && !isLastPage()) {
            if (visibleItemCount + firstVisibleItemPosition + STOCK_FOR_LOADING_TIME >= totalItemCount && firstVisibleItemPosition >= 0) {
                loadMoreItems()
            }
        }
    }
    ...
    companion object{
        private const val STOCK_FOR_LOADING_TIME = 10
    }
}